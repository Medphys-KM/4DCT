...
class AffineTransorform(object):
    ...
    def random(self):
        self.Affine_angle = np.random.uniform(0.0, 0.0)
    ...
    def run(self, img, status=False):
        if status:
            self.random()
        if self.flag:
            img = functional.ShiftScaleRotate(img, self.Affine_angle, self.Affine_scale, self.Affine_dx, self.Affine_dy,interpolation=cv2.INTER_LINEAR, border_mode=cv2.BORDER_CONSTANT)
...
class ElasticDistortionTransorform(object):
    ...
    def run(self, img, status=False):
        if status:
            self.random()
        if self.flag:
            #img=functional.elastic_transform_approx(img, self.ElasticDistortion_alpha, self.ElasticDistortion_sigma, self.ElasticDistortion_alpha_affine, self.ElasticDistortion_interpolation, cv2.BORDER_REFLECT_101, None, np.random.RandomState(self.ElasticDistortion_random_state), False)
            ...
            img=functional.elastic_transform_approx(img, self.ElasticDistortion_alpha, self.ElasticDistortion_sigma, self.ElasticDistortion_alpha_affine, self.ElasticDistortion_interpolation, cv2.BORDER_REFLECT_101)
